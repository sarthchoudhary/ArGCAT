...
# for pretrigger_sum_UpperThreshold in pretrigger_sum_UpperThreshold_ls:
    ...
    ax_2[4].plot(bin_edges[:-1], 1.-ratio_3_0, alpha=0.2, color='#e59acf', label = 'overall')
...
pickle.dump(fig_5, open(path.join(output_subdir, 'cut3_eff_comparison.pkl'),  'wb') )
fig_5.savefig(path.join(output_subdir, 'cut3_eff_comparison.pdf'))
# pickle.dump(fig_5, open(path.join(output_subdir, 'cut2_eff_comparison.pkl'),  'wb') )
...
# pickle.dump(fig_5, open(path.join(output_subdir, 'cut1_eff_comparison.pkl'),  'wb') )
plt.close(fig_5)
