   ...
   '''
    ...
    # for event_index in trange(100, colour='blue'): # diag
        ...
        for ch in range(3): ## we always search in all channels.
            mean_peak_loc = np.ceil(np.mean(np.where(flt_above_3rms[ch] != 0))) # np.mean(find_peaks(flt_above_3rms[ch])[0])
            if  (mean_peak_loc <= 500) and (mean_peak_loc >= 400): # peak in appropriate location.
                #TODO: it should be such that only events where all channels satisfy this condition should be selected.
                event_dict = {
                    'event_counter': event_index+1,
                    wf_str_ls[ch]: og_wf[ch],
                    filtered_wf_str_ls[ch]: mas[ch],
                    peak_str_ls[ch]: mean_peak_loc,
...
def main(file_config: dict, ch_number_ls:list[int], plots_target:int, save_plots:bool=True) ->None:
    ...
    '''
    get_basename = lambda filename: '_'.join(filename.replace('_', '.').split(sep='.')[2:-1:])
